...
namespace WfComponent
{
    ...
    internal sealed class RequestCommand
    {
        ...
        internal bool ExecuteWSLCommand(string command, string arguments, ref string stdout, ref string stderr, string workdir = null)
        {
            /// var args = "bash -c \" "
           var args =  " --distribution " + ConstantValues.WSLname   // 利用するWSL登録名
                            + " --user " + ConstantValues.WSLUser   // 使うWSLには ユーザ名が user で作成している。
                            + " -- "
                            + "source ~/.profile; "
                            + command
                            + " "
